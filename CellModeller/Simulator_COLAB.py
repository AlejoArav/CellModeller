...
"""
    ...
    def __init__(self, \
                    ...
                    dt, \
                    pickleSteps=50, \
                    outputDirName="/content/CellModeller/data", \
                    moduleStr=None, \
                    saveOutput=False, \
                    clPlatformNum=0, \
                    clDeviceNum=0, \
    ...
    def init_data_output(self):
        ...
        outputFileRoot = self.moduleName + '-' + time.strftime('%y-%m-%d-%H-%M', startTime)
        self.outputDirPath = os.path.join(self.outputDirName, outputFileRoot)
        
        # Alejandro Aravena - Added below to check whether directory exists.
        # Check whether the directory exists
        if not os.path.exists(self.outputDirPath):
            # If not, create it
            os.makedirs(self.outputDirPath)

            # If it exists, create the directory but add a number to the end
        else:
            i = 1
            while os.path.exists(self.outputDirPath + '_' + str(i)):
                i += 1
            self.outputDirPath += '_' + str(i)
            os.makedirs(self.outputDirPath)

        # --------------------------------
        # Alejandro Aravena - Modification -
        # Commented code below, it's not necessary for the simulation in Colab Environment
        # --------------------------------
        #if 'CMPATH' in os.environ:
